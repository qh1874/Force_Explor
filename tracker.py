...
class TrackerSWTS(Tracker2):
    def __init__(self, means, T, tau, store_rewards_arm=False):
        ...
        self.SS = np.zeros(self.nb_arms)
        self.N = np.zeros(self.nb_arms)
...
class SWTracker(Tracker2):
    ...
    def update(self, t, arm, reward):
        ...
        if self.store_rewards_arm:
            self.rewards_arm[arm].append(reward)
        #self.tau=int(self.T**0.5*np.log(3+self.t))
        if t >= self.tau:
            first_t = t - self.tau
            first_element = int(self.arm_sequence[first_t])
...
class SWTracker_exp(Tracker2):
    def __init__(self, means, T, tau, store_rewards_arm=False):
        super(SWTracker_exp, self).__init__(means, T, store_rewards_arm)
        self.tau = 0 #int(T**(1/3))#self.nb_arms #np.exp(np.log(np.log(T))/np.log(T))# #tau #int(np.log(T))+1
        self.time_changes = [0]
        self.S=np.zeros(self.nb_arms)
        self.N=np.zeros(self.nb_arms)
        self.S2=np.zeros(self.nb_arms)
        ...
        self.sum=self.tau
        self.tau0=tau
        self.Saa = np.zeros(self.nb_arms)
        self.Naa = np.zeros(self.nb_arms)
        self.count=0
        self.arm_s = -np.ones(self.T)
    ...
    def update(self, t, arm, reward):
        ...
        if np.sum(self.flag)==self.nb_arms:
            ...
            #self.Na=np.zeros(self.nb_arms)
            #self.tau = self.cnt**1
            #self.tau = self.tau0#/np.log(self.tau0)
            self.tau = (np.exp(1/np.log(self.tau0)))**self.cnt
            #self.tau=np.sqrt(self.T*np.log(self.T))
            self.cnt += 1
               
        if t >= self.tau0:
            first_t = t - self.tau0
            first_element = int(self.arm_sequence[first_t])
            self.Naa[first_element] -= 1
            self.Saa[first_element] -= self.reward[first_t]
...
class SWTracker_lin(Tracker2):
    def __init__(self, means, T, tau, store_rewards_arm=False):
        super(SWTracker_lin, self).__init__(means, T, store_rewards_arm)
        self.tau = 0 #int(T**(1/3))#self.nb_arms #np.exp(np.log(np.log(T))/np.log(T))# #tau #int(np.log(T))+1
        self.time_changes = [0]
        self.S=np.zeros(self.nb_arms)
        self.N=np.zeros(self.nb_arms)
    ...
    def update(self, t, arm, reward):
        ...
        if np.sum(self.flag)==self.nb_arms:
            ...
            #self.Na=np.zeros(self.nb_arms)
            self.tau = self.cnt**1
            #self.tau = np.sqrt(self.tau0)#/np.log(self.tau0)
            #self.tau = np.exp(1/np.log(self.tau0))**self.cnt
            #self.tau=np.sqrt(self.T*np.log(self.T))
            self.cnt += 1
            
            #if self.tau>self.tau0:#/np.log(self.tau0):
            # if any(self.Naa<np.log(self.tau0)):
            #     # self.cnt=0
            #     # self.tau = 0
            #     # self.count += 1
            #     pass
...
class SWTracker_con(Tracker2):
    def __init__(self, means, T, tau, store_rewards_arm=False):
        super(SWTracker_con, self).__init__(means, T, store_rewards_arm)
        self.tau = 0 #int(T**(1/3))#self.nb_arms #np.exp(np.log(np.log(T))/np.log(T))# #tau #int(np.log(T))+1
        self.time_changes = [0]
        self.S=np.zeros(self.nb_arms)
        self.N=np.zeros(self.nb_arms)

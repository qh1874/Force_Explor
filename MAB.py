...
from tracker import SWTracker, DiscountTracker, Tracker2, TrackerDTS1, TrackerEXP3, TrackerDTS, TrackerCUSUM, TrackerMUCB, TrackerTS,SWTracker_con,SWTracker_exp,SWTracker_lin
...
mapping = {'B': arms.ArmBernoulli, 'beta': arms.ArmBeta, 'F': arms.ArmFinite, 'G': arms.ArmGaussian,
           'Exp': arms.ArmExponential,'Pareto':arms.ArmPareto, 'dirac': arms.dirac, 'TG': arms.ArmTG}
...
def mom(rewards_arm,delta):
    """median-of-means estimator (Bubeck, Cesa-Bianchi,and Lugosi 2013)

    Args:
        rewards_arm (_type_): _description_
        delta (_type_): _description_

    Returns:
        _array_: median-of-means estimator
    """
    n=len(rewards_arm)
    m=max(int(min(8*np.log(np.exp(1/8)/delta),n/2)),1)
    N=int(n/m)
    mu_hat=np.zeros(m)
    # if N!=2:
    #     print("hello")
    for i in range(m):
        mu_hat[i]=np.mean(rewards_arm[i*N:(i+1)*N])
    return np.median(mu_hat)
...
class GenericMAB:
    ...
    def TS_gaussian(self, T, store_rewards_arm=True):
        ...
        for t in range(T):
            ...
            if all(tr.nb_draws)==False:
                sigma_hat=1e12
            else:
                sigma_hat=(1/np.maximum(tr.nb_draws-1,1)*(tr.S2-tr.nb_draws*(tr.S/tr.nb_draws)**2))**1
            sigma=4*sigma_hat/(tr.nb_draws+1)
            tr.update(t, arm, reward)
        return tr
    ...
    def SW_TS_gaussian(self, T, tau, mu_0, sigma_0, sigma, store_rewards_arm=True):
        ...
        for t in range(T):
            self.check_restart(tr)
            sigma_square = 1 / (1e-12 / sigma_0 ** 2 + tr.nb_draws / sigma ** 2)
            mu_ = sigma_square * (mu_0 / sigma_0 ** 2 + tr.S / sigma ** 2)
            arm = np.argmax(np.random.normal(mu_, np.sqrt(sigma_square)))
            reward = self.MAB[arm].sample()[0]
            tr.update(t, arm, reward)
    ...
    def SW_TS_gaussian_new(self, T, tau, mu_0, sigma_0, sigma, store_rewards_arm=True):
        """
        Adaptation of SW_TS to Gaussian distributions
        :param T: T: T: time horizon.
        :param tau: Length of the sliding window used to compute the posterior
        ...
        """
        #tr = TrackerSWTS(self.means, T, tau, store_rewards_arm=store_rewards_arm)
        tr = SWTracker_exp(self.means, T, tau=tau, store_rewards_arm=store_rewards_arm)
        K=tr.nb_arms
        tao=np.ones(K)
        mu_hat=np.zeros(K)
        for t in range(T):
            self.check_restart(tr)
            if all(tr.tau>tr.Na)==False: # tr.tau>tr.Na
                arm=np.argmin(tr.tau-tr.Na)
            else:
                arm = np.argmax(np.random.normal(mu_hat, tao))
            reward = self.MAB[arm].sample()[0]
            tr.update(t, arm, reward)
    ...
    def Index_Policy(self, T, index_func, tau=None, start_explo=1, store_rewards_arm=True):
        ...
        return tr
    # INDEX POLICIES
    def Index_Policy1(self, T, index_func, tau=None, Tracker=None,start_explo=1, store_rewards_arm=True):
        """
        Implementation of UCB1 algorithm.
        :param T: int, time horizon.
        :param index_func: function which computes the index with the tracker.
        ...
        if tau is None:
            tau = T
        ch=np.ones(T)*(-1)
        ch1=np.zeros(self.nb_arms)
        ch2=np.zeros(self.nb_arms)
        if Tracker is None:
            Tracker=Tracker2
        tr = Tracker(self.means, T, tau=tau, store_rewards_arm=store_rewards_arm)
        for t in range(T):
            ...
            reward = self.MAB[arm].sample()[0]  
            tr.update(t, arm, reward)
            if arm==np.argmax(tr.means[-1]):
                a=0
            if tr.tau>=tr.nb_arms:
                a=0
            if t%100==0:
                a=0
            if t%1000==0: 
    ...
    def Index_Policy_greedy(self, T, index_func, tau=None, Tracker=None,start_explo=1, store_rewards_arm=False):
        """
        ...
        for t in range(T):
            ...
            self.check_restart(tr)
            if np.random.uniform(0,1)<(t+1)**(-1/3): # tr.tau>tr.Na
                arm=np.random.randint(0,self.nb_arms)
            else:
                arm = rd_argmax(index_func(tr))
    ...
    def Index_Policy_Lattimore(self, T, index_func, tau=None, Tracker=None,start_explo=1, store_rewards_arm=False):
        """
        Implementation of epsilon greedy algorithm.
        :param T: int, time horizon.
        :param index_func: function which computes the index with the tracker.
    ...
    def Index_Policy_SW(self, T, index_func, tau=None, Tracker=None,start_explo=1, store_rewards_arm=False):
        """
        Implementation of our method in non-stationary settings.
        :param T: int, time horizon.
        :param index_func: function which computes the index with the tracker.
        :param tau: length of the sliding window if set.
        :param start_explo: number of time to explore each arm before comparing index.
        ...
        ch1=np.zeros(self.nb_arms)
        ch2=np.zeros(self.nb_arms)
        ch2t=np.zeros(self.nb_arms)
        Ns=np.zeros((T,self.nb_arms))
        ...
        for t in range(T):
            ...
            # if t>=tr.tau0:
            #     first_t = t - tr.tau0
            #     if tr.arm_s[first_t] != -1:
            #         first_element = int(tr.arm_sequence[first_t])
            #         ch2t[first_element] -= 1
            #Ns[t]=ch2t
            # if arm==np.argmax(tr.means[-1]):
            #     a=0
            # if t%tr.tau0==0:
    ...
    def Index_Policy_Discount(self, T, index_func, gamma, start_explo=1, store_rewards_arm=False):
        """
    ...
    def SW_UCB(self, T, tau, C=1.):
        ...
        def index_func(x):
            ...
            return x.Sa / np.maximum(x.Na,1) + C * np.sqrt(np.log(min(x.t+1, tau)) / np.maximum(x.Na,1))
        
        return self.Index_Policy(T, index_func, tau)

    def FE_EXP_SW(self, T, tau, C=1.):
        """
        Implementation of our method (exponential sequence) in non-stationary settings
        :param T: Time Horizon.
        :param tau: length of the sliding window, all information kept when tau = T.
        :return:
        """
        def index_func(x):
            #return x.Sa / (x.Na + 1e-12) + 2 * np.sqrt(max(min(np.log(x.t), tau), 1) / (x.Na + 1e-12))
            # xtemp=np.zeros(self.nb_arms)
            # for i in range(self.nb_arms):
            #     xtemp[i]=mom(x.rewards_arm[i],1/(x.t+1)**2)
            #return x.S / x.N  + 2*(1/np.maximum(x.N-1,1)*(x.S2-x.N*(x.S/x.N)**2))**0.5 * np.sqrt( np.log((1+x.t)**2) / (x.N + 1e-12))
            
            return x.Saa / (x.Naa + 1e-12) 
           
        return self.Index_Policy_SW(T, index_func, tau,SWTracker_exp)

    def FE_LIN_SW(self, T, tau, C=1.):
        """
        Implementation of our method (linear sequence) in non-stationary settings
        :param T: Time Horizon.
        :param tau: length of the sliding window, all information kept when tau = T.
        :param C: leverage in the upper bound (tunable param/support).
        :return:
        ...
        return self.Index_Policy_SW(T, index_func, tau,SWTracker_lin)
    
    def FE_CON_SW(self, T, tau, C=1.):
        """
        Implementation of our method (constant sequence) in non-stationary settings
        :param T: Time Horizon.
        :param tau: length of the sliding window, all information kept when tau = T.
        :param C: leverage in the upper bound (tunable param/support).
        :return:
        ...
        return self.Index_Policy_SW(T, index_func, tau,SWTracker_con)

    def FE_exp(self, T):
        """
        Our method: exponential exploration sequence
        :param T: Time Horizon.
        :param tau: length of the sliding window, all information kept when tau = T.
        :param C: leverage in the upper bound (tunable param/support).
        :return:
        ...
        def index_func(x):
            ...
            return x.Saa / np.maximum(x.Naa,1) 
        
        return self.Index_Policy1(T, index_func, T,SWTracker_exp)
    
    def FE_lin(self, T):
        """
        Our method: linear exploration sequence
        :param T: Time Horizon.
        :param tau: length of the sliding window, all information kept when tau = T.
        :param C: leverage in the upper bound (tunable param/support).
        :return:
        ...
        return self.Index_Policy1(T, index_func, T,SWTracker_lin)
    
    def FE_con(self, T):
        """
        Our method: constant exploration sequence
        :param T: Time Horizon.
        :param tau: length of the sliding window, all information kept when tau = T.
        :param C: leverage in the upper bound (tunable param/support).
        :return:
        ...
        return self.Index_Policy1(T, index_func, T,SWTracker_con)
    
    def Lattimore(self, T):
        """
        Implementation of On Explore-Then-Commit Strategies (Lattimore 2017)
        :param T: Time Horizon.
        :param tau: length of the sliding window, all information kept when tau = T.
        :param C: leverage in the upper bound (tunable param/support).
        :return:
        ...
        def index_func(x):
            ...
            #return x.S / (x.N+1e-12)  + 2*(x.S2/(x.N+1e-12)-(x.S/(x.N+1e-12))**2)**0.5 * np.sqrt( np.log(T) / (x.N + 1e-12))

            return x.S / (x.N+1e-12)  + np.sqrt(12*16/8)*(x.S2/(x.N+1e-12)-(x.S/(x.N+1e-12))**2)**0.5 * np.sqrt( np.log(T) / (x.N + 1e-12))
            
        return self.Index_Policy_Lattimore(T, index_func, T, SWTracker_con)
    ...
    def epsilon_greedy(self, T):
        ...
        def index_func(x):
            ...
            return x.S / (x.N +1e-12)
            
        return self.Index_Policy_greedy(T, index_func, T,SWTracker_con)
    ...
    def DS_UCB(self, T, gamma, B, ksi):
        ...
        """
        def index_func(x):
            return x.Sa/(x.Na+1e-12) + 2 * 30 * np.sqrt(ksi*np.log(np.sum(x.Na))/(x.Na + 1))

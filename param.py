...
param={
    'T':100000, # round 
    'K':5,  # arm
    'm':25000, # length of stationary phase, breakpoints=T/m
    'N':10 # repeat times
}
...
tau_theorique = np.sqrt(T/Gamma_T_garivier*np.log(T))#2*reward_u_p*np.sqrt(T*np.log(T)/Gamma_T_garivier)
tau_theorique_unb = np.sqrt(np.log(T)*T/Gamma_T_garivier)#2*(reward_u_p + 2*sigma_max)*np.sqrt(T*np.log(T)/Gamma_T_garivier)
...
param_lbsda={'tau': int(tau_theorique_unb)}
#param_lbsda={}
param_cumsum={'alpha':alpha_CUSUM , 'h': h_CUSUM, 'M':M_CUSUM, 'eps':eps_CUSUM, 'ksi':1/2}
param_mucb={'w':w_BRANO, 'b':b_BRANO, 'gamma':gamma_MUCB}
param_fe_exp={'tau': int(np.sqrt(T/Gamma_T_garivier)*np.log(T))}
param_fe_lin={'tau': int(np.sqrt(T/Gamma_T_garivier*np.log(T)))}
param_fe_con={'tau': int(np.sqrt(T/Gamma_T_garivier*np.log(T)))}